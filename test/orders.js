...
function testMem3Blt() {
    ...
    // 7-bit encoded field flags: 0xFFFF -> (0x7F | 0x80), (0x7F | 0x80), 0x03
    // Since TS_ZERO_FIELD_BYTE_BIT0 is set, we read 3 - 1 = 2 bytes.
    // Bytes: 0xFF, 0xFF. Resulting flags: 0x7F | (0x7F << 7) = 0x3FFF (first 14 fields)
    // To get all 16 fields, we would need 3 bytes, but here we test 14 fields.
    // Fields:
    //   cacheId: 1
    //   nLeftRect: 10
    //   nTopRect: 10
...
function testMultiOpaqueRect() {
    ...
    //   blue: 0x00
    //   numRectangles: 1
    //   rectData: Two-Byte Header Variable Field containing Delta-Encoded Rectangles
    //     cbData: 2 bytes
    //     rgbData:
    ...
    // Field Flags 7-bit Encoded: 0x01FF -> 0xFF (0x7F | 0x80), 0x03
    var buffer = Buffer.from([
        0x09, 0x12, 0xFF, 0x03,
        0x0A, 0x00, 0x0A, 0x00, 0x64, 0x00, 0x64, 0x00,
        0xFF, 0x00, 0x00,
...
function testPolygonSC() {
    ...
    //   brushColor: 0x0000FF (Blue)
    //   nDeltaEntries: 2
    //   points:
    //     cbData: 3 bytes
    //     rgbData:
    //       zeroBits: ceil(2*2/8) = 1 byte. 0x00
    //       deltaEncodedPoints:
    //         x1Delta: 5 (0x05)
    //         y1Delta: 5 (0x05)
    //         x2Delta: 10 (0x0A)
    ...
    assert.strictEqual(order.fields.xStart, 10);
    assert.strictEqual(order.fields.brushColor, 0x0000FF);
    assert.strictEqual(order.fields.points.length, 2);
    assert.strictEqual(order.fields.points[0].x, 5);
    assert.strictEqual(order.fields.points[1].x, 15);
...
function testPolyline() {
    ...
    //   yStart: 50
    //   bRop2: 0x0D
    //   penColor: 0x00FF00 (Green)
    //   numPoints: 1
    //   points:
    //     cbData: 2 bytes
    //     rgbData:
    //       zeroBits: 1 byte (0x00)
    //       deltaEncodedPoints:
    //         x1Delta: 20
    //         y1Delta: 20
    ...
    assert.strictEqual(order.fields.points[0].y, 20);
