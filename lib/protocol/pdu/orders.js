...
OrderParser.prototype.parseDeltaRects = function(s, count) {
    var rects = [];
    for (var i = 0; i < count; i++) {
        rects.push({
            left: this.readPackedValue(s),
            top: this.readPackedValue(s),
            width: this.readPackedValue(s),
            height: this.readPackedValue(s)
        });
    }
    return rects;
};
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    // MultiOpaqueRect uses the same state as OpaqueRect
    var opaqueState = this.orderState[OrderType.TS_NEG_OPAQUERECT_INDEX];

    if (fieldFlags & 0x01) opaqueState.nLeftRect = this.readCoord(s, delta, opaqueState.nLeftRect);
    if (fieldFlags & 0x02) opaqueState.nTopRect = this.readCoord(s, delta, opaqueState.nTopRect);
    if (fieldFlags & 0x04) opaqueState.nWidthRect = this.readCoordUnsigned(s, delta, opaqueState.nWidthRect);
    if (fieldFlags & 0x08) opaqueState.nHeightRect = this.readCoordUnsigned(s, delta, opaqueState.nHeightRect);
    if (fieldFlags & 0x10) {
        var b = s.buffer.readUInt8(s.offset);
        var g = s.buffer.readUInt8(s.offset + 1);
        var r = s.buffer.readUInt8(s.offset + 2);
        s.offset += 3;
        opaqueState.colorIndex = (r << 16) | (g << 8) | b;
    }
    if (fieldFlags & 0x20) { opaqueState.numRectangles = s.buffer.readUInt8(s.offset); s.offset++; }
    if (fieldFlags & 0x40) { opaqueState.cbData = s.buffer.readUInt8(s.offset); s.offset++; }
    if (fieldFlags & 0x80) {
        var startOffset = s.offset;
        opaqueState.rectData = this.parseDeltaRects(s, opaqueState.numRectangles || 0);
        var readLength = s.offset - startOffset;
        if (readLength !== opaqueState.cbData) {
            log.warn("MultiOpaqueRect: read " + readLength + " bytes, but cbData is " + opaqueState.cbData);
            s.offset = startOffset + opaqueState.cbData;
        }
    }

    Object.assign(fields, opaqueState);
};
