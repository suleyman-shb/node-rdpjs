...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x40) {
        ...
        state.rectangles = this.parseDeltaRects(s, state.numRectangles);
        if (s.offset - startOffset !== state.cbData) {
            log.warn('MultiOpaqueRect: cbData (' + state.cbData + ') does not match read data length (' + (s.offset - startOffset) + ')');
