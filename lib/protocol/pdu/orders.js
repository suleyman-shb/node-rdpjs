...
OrderParser.prototype.readPackedValue = function(s) {
    ...
    if (isTwoBytes) {
        result = (result << 8) | s.buffer.readUInt8(s.offset);
        s.offset++;
    }
...
OrderParser.prototype.parseDeltaRects = function(s, cbData, baseRect) {
    var end = s.offset + cbData;
    var rects = [];
    var lastRect = {
        left: baseRect.nLeftRect,
        top: baseRect.nTopRect,
        width: baseRect.nWidthRect,
        height: baseRect.nHeightRect
    };

    while (s.offset < end) {
        var rect = {
            left: lastRect.left + this.readPackedValue(s),
            top: lastRect.top + this.readPackedValue(s),
            width: lastRect.width + this.readPackedValue(s),
            height: lastRect.height + this.readPackedValue(s)
        };
        rects.push(rect);
        lastRect = rect;
    }
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x20) { state.cbData = s.buffer.readUInt8(s.offset); s.offset++; }

    Object.assign(fields, state);

    if (fieldFlags & 0x40) {
        fields.rects = this.parseDeltaRects(s, state.cbData, state);
    }
};
