...
OrderParser.prototype.parsePrimaryOrder = function(s, controlFlags) {
    ...
    if (!(controlFlags & ControlFlags.TS_ZERO_FIELD_COUNT)) {
        var byte = s.buffer.readUInt8(s.offset);
        s.offset++;
        fieldFlags = byte & 0x7F;
        var shift = 7;
        while (byte & 0x80) {
            byte = s.buffer.readUInt8(s.offset);
            s.offset++;
            fieldFlags |= (byte & 0x7F) << shift;
            shift += 7;
        }
    }
...
OrderParser.prototype.readPackedValue = function(s) {
    var byte = s.buffer.readUInt8(s.offset);
    s.offset++;
    var isNegative = !!(byte & 0x40);
    var isTwoByte = !!(byte & 0x80);
    var value = byte & 0x3F;

    if (isTwoByte) {
        value = (value << 8) | s.buffer.readUInt8(s.offset);
        s.offset++;
    }
...
OrderParser.prototype.parseDeltaRects = function(s, numRects, baseRect) {
    var rects = [];
    var prevRect = baseRect;

    for (var i = 0; i < numRects; i++) {
        var rect = {
            left: prevRect.left + this.readPackedValue(s),
            top: prevRect.top + this.readPackedValue(s),
            right: prevRect.right + this.readPackedValue(s),
            bottom: prevRect.bottom + this.readPackedValue(s)
        };
        rects.push(rect);
        prevRect = rect;
    }
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x20) { state.numRectangles = s.buffer.readUInt8(s.offset); s.offset++; }
    if (fieldFlags & 0x40) {
        var cbData = s.buffer.readUInt8(s.offset);
        s.offset++;
        var startOffset = s.offset;
        state.rectData = this.parseDeltaRects(s, state.numRectangles, {
            left: state.nLeftRect,
            top: state.nTopRect,
            right: state.nLeftRect + state.nWidthRect - 1,
            bottom: state.nTopRect + state.nHeightRect - 1
        });
        if (s.offset - startOffset !== cbData) {
            log.warn('MultiOpaqueRect: read ' + (s.offset - startOffset) + ' bytes, expected ' + cbData);
    ...
    // MultiOpaqueRect and OpaqueRect should share state for coordinates and color
    var opaqueState = this.orderState[OrderType.TS_NEG_OPAQUERECT_INDEX];
    opaqueState.nLeftRect = state.nLeftRect;
    opaqueState.nTopRect = state.nTopRect;
    opaqueState.nWidthRect = state.nWidthRect;
    opaqueState.nHeightRect = state.nHeightRect;
    opaqueState.colorIndex = state.colorIndex;
};
