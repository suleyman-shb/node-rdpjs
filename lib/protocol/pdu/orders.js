...
OrderParser.prototype.parsePrimaryOrder = function(s, controlFlags) {
    ...
    if (!(controlFlags & ControlFlags.TS_ZERO_FIELD_COUNT)) {
        ...
        if (fieldFlags & 0x80) {
            fieldFlags &= 0x7F;
            fieldFlags |= (s.buffer.readUInt8(s.offset) << 7);
            s.offset++;
            if (fieldFlags & 0x4000) {
                fieldFlags &= 0x3FFF;
                fieldFlags |= (s.buffer.readUInt8(s.offset) << 14);
                s.offset++;
            }
        }
    }
...
OrderParser.prototype.readPackedValue = function(s) {
    var val = s.buffer.readUInt8(s.offset);
    s.offset++;
    var isNegative = !!(val & 0x40);
    var isTwoBytes = !!(val & 0x80);

    if (isTwoBytes) {
        val = ((val & 0x3F) << 8) | s.buffer.readUInt8(s.offset);
        s.offset++;
    } else {
        val &= 0x3F;
    }
