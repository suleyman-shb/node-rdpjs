...
OrderParser.prototype.parseDeltaRects = function(s, numRectangles, baseRect) {
    var rects = [];
    var prev = {
        left: baseRect.nLeftRect || 0,
        top: baseRect.nTopRect || 0,
        width: baseRect.nWidthRect || 0,
        height: baseRect.nHeightRect || 0
    };
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x80) {
        ...
        state.rectangles = this.parseDeltaRects(s, state.numRectangles || 0, state);
        var readLength = s.offset - startOffset;
        var cbData = state.cbData || 0;
        if (readLength !== cbData) {
            log.warn("MultiOpaqueRect cbData mismatch: expected " + cbData + ", read " + readLength);
