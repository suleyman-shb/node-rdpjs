...
OrderFieldsCount[OrderType.TS_NEG_OPAQUERECT_INDEX] = 5;
OrderFieldsCount[OrderType.TS_NEG_SAVEBITMAP_INDEX] = 6;
OrderFieldsCount[OrderType.TS_NEG_MULTIOPAQUERECT_INDEX] = 7;
OrderFieldsCount[OrderType.TS_NEG_POLYGON_SC_INDEX] = 9;
OrderFieldsCount[OrderType.TS_NEG_POLYLINE_INDEX] = 7;
...
OrderParser.prototype.parsePrimaryOrder = function(s, controlFlags) {
    ...
    var maxFieldFlagsBytes = Math.floor((numFields + 7) / 8);
    ...
    switch(orderType) {
        ...
        case OrderType.TS_NEG_MULTIOPAQUERECT_INDEX:
            // MultiOpaqueRect and OpaqueRect share the same internal coordinate and color state
            var opaqueRectState = this.orderState[OrderType.TS_NEG_OPAQUERECT_INDEX];
            this.parseMultiOpaqueRect(s, controlFlags, fieldFlags, order.fields, opaqueRectState);
            break;
        case OrderType.TS_NEG_SAVEBITMAP_INDEX:
            this.parseSaveBitmap(s, controlFlags, fieldFlags, order.fields, state);
            break;
        case OrderType.TS_NEG_POLYGON_SC_INDEX:
            this.parsePolygonSC(s, controlFlags, fieldFlags, order.fields, state);
            break;
        case OrderType.TS_NEG_POLYLINE_INDEX:
            this.parsePolyline(s, controlFlags, fieldFlags, order.fields, state);
            break;
        default:
...
OrderParser.prototype.parseCodedDeltaList = function(s, numEntries, bitsPerEntry, numDeltasPerEntry, initialValues) {
    var zeroBitsBytesCount = Math.ceil((numEntries * bitsPerEntry) / 8);
    var zeroBits = s.buffer.slice(s.offset, s.offset + zeroBitsBytesCount);
    s.offset += zeroBitsBytesCount;

    var entries = [];
    var prev = (initialValues || new Array(numDeltasPerEntry).fill(0)).slice();

    for (var i = 0; i < numEntries; i++) {
        var bitOffset = i * bitsPerEntry;
        var byteIdx = Math.floor(bitOffset / 8);
        var bitIdxInByte = bitOffset % 8;

        var entryBits;
        if (bitsPerEntry === 4) {
            entryBits = (zeroBits[byteIdx] >> (4 - bitIdxInByte)) & 0x0F;
        } else if (bitsPerEntry === 2) {
            entryBits = (zeroBits[byteIdx] >> (6 - bitIdxInByte)) & 0x03;
        } else {
            entryBits = 0;
        }

        var entry = [];
        for (var j = 0; j < numDeltasPerEntry; j++) {
            var isZero = !!(entryBits & (1 << (numDeltasPerEntry - 1 - j)));
            var delta = isZero ? 0 : this.readPackedValue(s);
            var val = prev[j] + delta;
            entry.push(val);
            prev[j] = val;
        }
        entries.push(entry);
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x10) {
        ...
        state.colorIndex = (r << 16) | (g << 8) | b;
    }
    if (fieldFlags & 0x20) state.numRectangles = s.buffer.readUInt8(s.offset++);
    if (fieldFlags & 0x40) {
        var cbData = s.buffer.readUInt8(s.offset++);
        var startOffset = s.offset;
        var initial = [state.nLeftRect, state.nTopRect, state.nWidthRect, state.nHeightRect];
        state.rects = this.parseCodedDeltaList(s, state.numRectangles, 4, 4, initial).map(r => ({
            left: r[0], top: r[1], width: r[2], height: r[3]
        }));
        if (s.offset - startOffset !== cbData) {
            log.warn('MultiOpaqueRect: read ' + (s.offset - startOffset) + ' bytes, but cbData was ' + cbData);
            s.offset = startOffset + cbData;
        }
    }
...
OrderParser.prototype.parseDstBlt = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x04) state.nWidthRect = this.readCoordUnsigned(s, delta, state.nWidthRect);
    if (fieldFlags & 0x08) state.nHeightRect = this.readCoordUnsigned(s, delta, state.nHeightRect);
    if (fieldFlags & 0x10) { state.bRop3 = s.buffer.readUInt8(s.offset++); }
...
OrderParser.prototype.parseLineTo = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x0001) { state.mixMode = s.buffer.readUInt8(s.offset++); }
    ...
    if (fieldFlags & 0x0040) { state.bRop2 = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x0080) { state.penStyle = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x0100) { state.penWidth = s.buffer.readUInt8(s.offset++); }
...
OrderParser.prototype.parseSaveBitmap = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x20) { state.operation = s.buffer.readUInt8(s.offset++); }
...
OrderParser.prototype.parsePatBlt = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x0010) { state.bRop3 = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x0020) {
        var b = s.buffer.readUInt8(s.offset++);
        var g = s.buffer.readUInt8(s.offset++);
        var r = s.buffer.readUInt8(s.offset++);
    ...
    // Brush
    if (fieldFlags & 0x0080) { state.brushX = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x0100) { state.brushY = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x0200) { state.brushStyle = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x0400) { state.brushHatch = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x0800) {
        state.brushExtra = Buffer.from(s.buffer.slice(s.offset, s.offset + 7));
        s.offset += 7;
    }
...
OrderParser.prototype.parseMemBlt = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x0020) { state.bRop3 = s.buffer.readUInt8(s.offset++); }
...
OrderParser.prototype.parseMem3Blt = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x0400) { state.brushX = s.buffer.readInt8(s.offset++); }
    if (fieldFlags & 0x0800) { state.brushY = s.buffer.readInt8(s.offset++); }
    if (fieldFlags & 0x1000) { state.brushStyle = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x2000) { state.brushHatch = s.buffer.readUInt8(s.offset++); }
...
OrderParser.prototype.parsePolygonSC = function(s, controlFlags, fieldFlags, fields, state) {
    var delta = !!(controlFlags & ControlFlags.TS_DELTA_COORDS);
    ...
    if (fieldFlags & 0x04) state.nWidthRect = this.readCoordUnsigned(s, delta, state.nWidthRect);
    if (fieldFlags & 0x08) state.nHeightRect = this.readCoordUnsigned(s, delta, state.nHeightRect);
    if (fieldFlags & 0x10) { state.bRop2 = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x20) { state.fillMode = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x40) {
        var b = s.buffer.readUInt8(s.offset++);
        var g = s.buffer.readUInt8(s.offset++);
        var r = s.buffer.readUInt8(s.offset++);
        state.brushColor = (r << 16) | (g << 8) | b;
    }
    if (fieldFlags & 0x80) { state.nPoints = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x0100) {
        var cbData = s.buffer.readUInt8(s.offset++);
        var startOffset = s.offset;
        state.points = this.parseCodedDeltaList(s, state.nPoints, 2, 2).map(p => ({ x: p[0], y: p[1] }));
        if (s.offset - startOffset !== cbData) {
            log.warn('PolygonSC: read ' + (s.offset - startOffset) + ' bytes, but cbData was ' + cbData);
            s.offset = startOffset + cbData;
        }
    }
...
OrderParser.prototype.parsePolyline = function(s, controlFlags, fieldFlags, fields, state) {
    var delta = !!(controlFlags & ControlFlags.TS_DELTA_COORDS);

    if (fieldFlags & 0x01) state.nXStart = this.readCoord(s, delta, state.nXStart);
    if (fieldFlags & 0x02) state.nYStart = this.readCoord(s, delta, state.nYStart);
    if (fieldFlags & 0x04) { state.bRop2 = s.buffer.readUInt8(s.offset++); }
    // fieldFlags & 0x08 is Reserved
    if (fieldFlags & 0x10) {
        var b = s.buffer.readUInt8(s.offset++);
        var g = s.buffer.readUInt8(s.offset++);
        var r = s.buffer.readUInt8(s.offset++);
    ...
    if (fieldFlags & 0x20) { state.nPoints = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x40) {
        var cbData = s.buffer.readUInt8(s.offset++);
        var startOffset = s.offset;
        var initial = [state.nXStart || 0, state.nYStart || 0];
        state.points = this.parseCodedDeltaList(s, state.nPoints, 2, 2, initial).map(p => ({ x: p[0], y: p[1] }));
        if (s.offset - startOffset !== cbData) {
            log.warn('Polyline: read ' + (s.offset - startOffset) + ' bytes, but cbData was ' + cbData);
            s.offset = startOffset + cbData;
        }
    }
