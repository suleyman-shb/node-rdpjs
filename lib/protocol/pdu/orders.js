...
function OrderParser() {
    ...
    // OpaqueRect and MultiOpaqueRect share the same state
    this.orderState[OrderType.TS_NEG_MULTIOPAQUERECT_INDEX] = this.orderState[OrderType.TS_NEG_OPAQUERECT_INDEX];
}
...
OrderParser.prototype.parseDeltaRects = function(s, numRectangles, baseRect) {
    var rects = [Object.assign({}, baseRect)];
    var current = Object.assign({}, baseRect);
    for (var i = 0; i < numRectangles; i++) {
        current.nLeftRect += this.readPackedValue(s);
        current.nTopRect += this.readPackedValue(s);
        current.nWidthRect += this.readPackedValue(s);
        current.nHeightRect += this.readPackedValue(s);
        rects.push(Object.assign({}, current));
    }
    return rects;
};
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x40) { state.cbData = s.buffer.readUInt8(s.offset); s.offset++; }

    Object.assign(fields, state);

    if (fields.numRectangles > 0) {
        var startOffset = s.offset;
        fields.rectangles = this.parseDeltaRects(s, fields.numRectangles, {
            nLeftRect: fields.nLeftRect,
            nTopRect: fields.nTopRect,
            nWidthRect: fields.nWidthRect,
            nHeightRect: fields.nHeightRect
        });
        var readDataLen = s.offset - startOffset;
        if (readDataLen !== fields.cbData) {
            log.warn("MultiOpaqueRect: cbData (" + fields.cbData + ") does not match read data length (" + readDataLen + "). Forcing sync.");
            s.offset = startOffset + fields.cbData;
        }
    } else {
        fields.rectangles = [{
            nLeftRect: fields.nLeftRect,
