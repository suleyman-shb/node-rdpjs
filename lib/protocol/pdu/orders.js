...
OrderFieldsCount[OrderType.TS_NEG_POLYGON_SC_INDEX] = 7;
...
OrderParser.prototype.parsePrimaryOrder = function(s, controlFlags) {
    ...
    if (fieldFlagsBytesToRead > 0) {
        ...
        fieldFlags = b & 0x7F;
        if (b & 0x80 && fieldFlagsBytesToRead > 1) {
            b = s.buffer.readUInt8(s.offset++);
            fieldFlags |= (b & 0x7F) << 7;
            if (b & 0x80 && fieldFlagsBytesToRead > 2) {
                b = s.buffer.readUInt8(s.offset++);
                fieldFlags |= (b & 0x7F) << 14;
            }
        }
    }
    ...
    switch(orderType) {
        ...
        case OrderType.TS_NEG_MULTIOPAQUERECT_INDEX:
            // MultiOpaqueRect and OpaqueRect share the same state
            this.parseMultiOpaqueRect(s, controlFlags, fieldFlags, order.fields, this.orderState[OrderType.TS_NEG_OPAQUERECT_INDEX]);
            break;
...
OrderParser.prototype.parseCodedDeltaList = function(s, numEntries, isRect, initialState) {
    var numDeltas = isRect ? numEntries * 4 : numEntries * 2;
    var zeroBitsBytesCount = Math.ceil(numDeltas / 8);
    var zeroBits = s.buffer.slice(s.offset, s.offset + zeroBitsBytesCount);
    s.offset += zeroBitsBytesCount;

    var entries = [];
    var current = Object.assign({}, initialState || (isRect ? { left: 0, top: 0, width: 0, height: 0 } : { x: 0, y: 0 }));

    for (var i = 0; i < numEntries; i++) {
        if (isRect) {
            var bitBase = i * 4;
            var leftDelta = ((zeroBits[Math.floor(bitBase / 8)] >> (bitBase % 8)) & 0x01) ? 0 : this.readPackedValue(s);
            var topDelta = ((zeroBits[Math.floor((bitBase + 1) / 8)] >> ((bitBase + 1) % 8)) & 0x01) ? 0 : this.readPackedValue(s);
            var widthDelta = ((zeroBits[Math.floor((bitBase + 2) / 8)] >> ((bitBase + 2) % 8)) & 0x01) ? 0 : this.readPackedValue(s);
            var heightDelta = ((zeroBits[Math.floor((bitBase + 3) / 8)] >> ((bitBase + 3) % 8)) & 0x01) ? 0 : this.readPackedValue(s);

            current.left += leftDelta;
            current.top += topDelta;
            current.width += widthDelta;
            current.height += heightDelta;
        } else {
            var bitBase = i * 2;
            var xDelta = ((zeroBits[Math.floor(bitBase / 8)] >> (bitBase % 8)) & 0x01) ? 0 : this.readPackedValue(s);
            var yDelta = ((zeroBits[Math.floor((bitBase + 1) / 8)] >> ((bitBase + 1) % 8)) & 0x01) ? 0 : this.readPackedValue(s);

            current.x += xDelta;
            current.y += yDelta;
        }
        entries.push(Object.assign({}, current));
    }
    return entries;
};
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x20) {
        state.numRectangles = s.buffer.readUInt8(s.offset);
        s.offset++;
    }
    if (fieldFlags & 0x40) {
        ...
        state.rects = this.parseCodedDeltaList(s, state.numRectangles, true, {
            left: state.nLeftRect,
            top: state.nTopRect,
            width: state.nWidthRect,
            height: state.nHeightRect
        });
...
OrderParser.prototype.parseMem3Blt = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x0400) { state.brushX = s.buffer.readUInt8(s.offset); s.offset++; }
    if (fieldFlags & 0x0800) { state.brushY = s.buffer.readUInt8(s.offset); s.offset++; }
...
OrderParser.prototype.parsePolygonSC = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x01) state.xStart = this.readCoord(s, delta, state.xStart);
    if (fieldFlags & 0x02) state.yStart = this.readCoord(s, delta, state.yStart);
    if (fieldFlags & 0x04) state.bRop2 = s.buffer.readUInt8(s.offset++);
    if (fieldFlags & 0x08) state.fillMode = s.buffer.readUInt8(s.offset++);
    if (fieldFlags & 0x10) {
        var b = s.buffer.readUInt8(s.offset++);
        var g = s.buffer.readUInt8(s.offset++);
        var r = s.buffer.readUInt8(s.offset++);
    ...
    if (fieldFlags & 0x20) state.numPoints = s.buffer.readUInt8(s.offset++);
    if (fieldFlags & 0x40) {
        var cbData = s.buffer.readUInt16LE(s.offset);
        s.offset += 2;
        var startOffset = s.offset;
        state.points = this.parseCodedDeltaList(s, state.numPoints, false, {
            x: state.xStart,
            y: state.yStart
        });
        if (s.offset - startOffset !== cbData) {
...
OrderParser.prototype.parsePolyline = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x10) state.numPoints = s.buffer.readUInt8(s.offset++);
    if (fieldFlags & 0x20) {
        var cbData = s.buffer.readUInt16LE(s.offset);
        s.offset += 2;
        var startOffset = s.offset;
...
OrderParser.prototype.parseSecondaryOrder = function(s, controlFlags) {
    ...
    var data = Buffer.from(s.buffer.slice(s.offset, s.offset + dataLength));
    s.offset += dataLength;
