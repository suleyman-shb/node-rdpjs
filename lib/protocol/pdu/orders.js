...
OrderFieldsCount[OrderType.TS_NEG_POLYLINE_INDEX] = 6;
...
OrderParser.prototype.parsePrimaryOrder = function(s, controlFlags) {
    ...
    switch(orderType) {
        ...
        case OrderType.TS_NEG_MULTIOPAQUERECT_INDEX:
            ...
            if (orderType === OrderType.TS_NEG_MULTIOPAQUERECT_INDEX) {
                this.parseMultiOpaqueRect(s, controlFlags, fieldFlags, order.fields, opaqueRectState);
            } else {
                this.parseOpaqueRect(s, controlFlags, fieldFlags, order.fields, opaqueRectState);
            }
            break;
...
OrderParser.prototype.parseCodedDeltaList = function(s, numEntries, useFourDeltas, prev) {
    var zeroBitsBytesCount = Math.ceil(numEntries / 2);
    var zeroBits = s.buffer.slice(s.offset, s.offset + zeroBitsBytesCount);
    s.offset += zeroBitsBytesCount;

    var entries = [];
    prev = prev || { left: 0, top: 0, width: 0, height: 0 };
    ...
    for (var i = 0; i < numEntries; i++) {
        ...
        if (useFourDeltas) {
            ...
            var xDelta = (bits & 0x08) ? 0 : this.readPackedValue(s);
            var yDelta = (bits & 0x04) ? 0 : this.readPackedValue(s);
...
OrderParser.prototype.parseMem3Blt = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x8000) {
        state.cacheIndex = s.buffer.readUInt16LE(s.offset);
        s.offset += 2;
    }
