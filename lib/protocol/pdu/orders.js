...
function OrderParser() {
    ...
    // Shared state between OpaqueRect and MultiOpaqueRect
...
OrderParser.prototype.parseDeltaRects = function(s, numRects, baseRect) {
    ...
    for (var i = 0; i < numRects; i++) {
        var rect = {
            ...
            width: (prevRect.width || 0) + this.readPackedValue(s),
            height: (prevRect.height || 0) + this.readPackedValue(s)
        };
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x40) {
        state.cbData = s.buffer.readUInt8(s.offset);
        s.offset++;
        var startOffset = s.offset;
        state.rects = this.parseDeltaRects(s, state.numRectangles, {
            left: state.nLeftRect,
            top: state.nTopRect,
            width: state.nWidthRect,
            height: state.nHeightRect
        });
        if (s.offset - startOffset !== state.cbData) {
            log.warn('MultiOpaqueRect cbData mismatch: expected ' + state.cbData + ' but read ' + (s.offset - startOffset));
            s.offset = startOffset + state.cbData;
        }
    }
