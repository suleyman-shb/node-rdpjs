...
OrderParser.prototype.readPackedValue = function(s) {
    var v = s.buffer.readUInt8(s.offset);
    s.offset++;
...
OrderParser.prototype.parseDeltaRects = function(s, firstRect, numRects) {
    var rects = [Object.assign({}, firstRect)];
    var prev = firstRect;
    for (var i = 1; i < numRects; i++) {
        var rect = {
            nLeftRect: prev.nLeftRect + this.readPackedValue(s),
            nTopRect: prev.nTopRect + this.readPackedValue(s),
            nWidthRect: prev.nWidthRect + this.readPackedValue(s),
            nHeightRect: prev.nHeightRect + this.readPackedValue(s)
        };
        rects.push(rect);
        prev = rect;
    }
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x20) {
        state.numRects = s.buffer.readUInt8(s.offset);
        s.offset++;
    }
    if (fieldFlags & 0x40) {
        var firstRect = {
            nLeftRect: state.nLeftRect,
            nTopRect: state.nTopRect,
            nWidthRect: state.nWidthRect,
            nHeightRect: state.nHeightRect
        };
        state.rects = this.parseDeltaRects(s, firstRect, state.numRects);
    }
...
OrderParser.prototype.parseMem3Blt = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    // Brush
    if (fieldFlags & 0x0100) { state.brushX = s.buffer.readUInt8(s.offset); s.offset++; }
    if (fieldFlags & 0x0200) { state.brushY = s.buffer.readUInt8(s.offset); s.offset++; }
    if (fieldFlags & 0x0400) { state.brushStyle = s.buffer.readUInt8(s.offset); s.offset++; }
    if (fieldFlags & 0x0800) { state.brushHatch = s.buffer.readUInt8(s.offset); s.offset++; }
    if (fieldFlags & 0x1000) {
        state.brushExtra = s.buffer.slice(s.offset, s.offset + 7);
        s.offset += 7;
    }
    if (fieldFlags & 0x2000) { state.cacheIndex = s.buffer.readUInt16LE(s.offset); s.offset += 2; }
