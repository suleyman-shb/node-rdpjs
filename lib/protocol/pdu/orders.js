...
function OrderParser() {
    ...
    // Share state between OpaqueRect and MultiOpaqueRect
...
OrderParser.prototype.parseDeltaRects = function(s, numRects) {
    var rects = [];
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x40) {
        ...
        state.rectData = this.parseDeltaRects(s, state.numRectangles || 0);
    ...
    if (state.rectData) {
        var last = rects[0];
        for (var i = 0; i < state.rectData.length; i++) {
            var r = state.rectData[i];
            var current = {
                left: last.left + r.left,
                top: last.top + r.top,
                width: last.width + r.width,
                height: last.height + r.height
            };
            rects.push(current);
            last = current;
        }
    }
    fields.rectangles = rects;
};
