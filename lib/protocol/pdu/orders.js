...
OrderParser.prototype.parseCodedDeltaList = function(s, numEntries, numDeltas) {
    var zeroBitsBytesCount = Math.ceil((numEntries * numDeltas) / 8);
    var zeroBits = s.buffer.slice(s.offset, s.offset + zeroBitsBytesCount);
    s.offset += zeroBitsBytesCount;

    var entries = [];
    var prev = new Array(numDeltas).fill(0);

    for (var i = 0; i < numEntries; i++) {
        var entry = [];
        for (var j = 0; j < numDeltas; j++) {
            var bitIdx = i * numDeltas + j;
            var byteIdx = Math.floor(bitIdx / 8);
            var bitInByte = bitIdx % 8;
            var isZero = !!(zeroBits[byteIdx] & (0x80 >> bitInByte));
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x0100) {
        ...
        state.rectData = this.parseCodedDeltaList(s, state.numRectangles, 4);
...
OrderParser.prototype.parsePolygonSC = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x0001) state.xStart = this.readCoord(s, delta, state.xStart);
    if (fieldFlags & 0x0002) state.yStart = this.readCoord(s, delta, state.yStart);
    ...
    if (fieldFlags & 0x0020) state.nDeltaEntries = s.buffer.readUInt8(s.offset++);
    if (fieldFlags & 0x0040) {
        var cbData = s.buffer.readUInt8(s.offset++);
        var startOffset = s.offset;
        state.points = this.parseCodedDeltaList(s, state.nDeltaEntries, 2);
        if (s.offset - startOffset !== cbData) {
...
OrderParser.prototype.parsePolyline = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x0010) state.numPoints = s.buffer.readUInt8(s.offset++);
    if (fieldFlags & 0x0020) {
        var cbData = s.buffer.readUInt8(s.offset++);
        var startOffset = s.offset;
        state.points = this.parseCodedDeltaList(s, state.numPoints, 2);
        if (s.offset - startOffset !== cbData) {
