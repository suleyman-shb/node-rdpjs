...
OrderParser.prototype.parseDeltaRects = function(s, num, base) {
    var rects = [];
    var left = base.nLeftRect || 0;
    var top = base.nTopRect || 0;
    var width = base.nWidthRect || 0;
    var height = base.nHeightRect || 0;

    // The first rectangle is the one from the primary order fields
    rects.push({
        left: left,
        top: top,
        width: width,
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x40) {
        state.cbData = s.buffer.readUInt8(s.offset); s.offset++;
        var startOffset = s.offset;
        state.rectangles = this.parseDeltaRects(s, state.numRectangles, state);
        if (s.offset !== startOffset + state.cbData) {
            log.warn("MultiOpaqueRect: cbData (" + state.cbData + ") does not match actual data size (" + (s.offset - startOffset) + ")");
