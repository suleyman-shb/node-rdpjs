...
OrderParser.prototype.parsePrimaryOrder = function(s, controlFlags) {
    ...
    var maxFieldFlagsBytes = Math.ceil(numFields / 7) || 1;
    ...
    for (var i = 0; i < fieldFlagsBytesToRead; i++) {
        var b = s.buffer.readUInt8(s.offset++);
        fieldFlags |= (b & 0x7F) << (7 * i);
    }
    ...
    switch(orderType) {
        ...
        case OrderType.TS_NEG_OPAQUERECT_INDEX:
            this.parseOpaqueRect(s, controlFlags, fieldFlags, order.fields, this.orderState[OrderType.TS_NEG_OPAQUERECT_INDEX]);
            break;
...
OrderParser.prototype.parseCodedDeltaList = function(s, numEntries, isRect) {
    var deltasPerEntry = isRect ? 4 : 2;
    var bitsPerEntry = isRect ? 4 : 2;
    ...
    var entries = [];
    var prev = isRect ? { left: 0, top: 0, width: 0, height: 0 } : { x: 0, y: 0 };

    for (var i = 0; i < numEntries; i++) {
        var bitOffset = i * bitsPerEntry;
        var byteIdx = Math.floor(bitOffset / 8);
        var shift = 8 - (bitOffset % 8) - bitsPerEntry;
        var bits = (zeroBits[byteIdx] >> shift) & ((1 << bitsPerEntry) - 1);
        ...
        if (isRect) {
            ...
            var xDelta = (bits & 0x02) ? 0 : this.readPackedValue(s);
            var yDelta = (bits & 0x01) ? 0 : this.readPackedValue(s);
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    state.colorIndex = (state.red << 16) | (state.green << 8) | state.blue;
    state.nWidth = state.nWidthRect; // For backward compatibility
    state.nHeight = state.nHeightRect; // For backward compatibility
...
OrderParser.prototype.parsePolygonSC = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x0001) state.nXStart = this.readCoord(s, delta, state.nXStart);
    if (fieldFlags & 0x0002) state.nYStart = this.readCoord(s, delta, state.nYStart);
    if (fieldFlags & 0x0004) { state.bRop2 = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x0008) { state.fillMode = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x0010) { state.brushIndex = s.buffer.readUInt8(s.offset++); }
    ...
    if (fieldFlags & 0x0080) { state.numPoints = s.buffer.readUInt8(s.offset++); }
    if (fieldFlags & 0x0100) {
        var cbData = s.buffer.readUInt16LE(s.offset);
        s.offset += 2;
        var startOffset = s.offset;
...
OrderParser.prototype.parsePolyline = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x08) { s.offset++; } // Reserved
    if (fieldFlags & 0x10) {
        var b = s.buffer.readUInt8(s.offset++);
        var g = s.buffer.readUInt8(s.offset++);
        var r = s.buffer.readUInt8(s.offset++);
