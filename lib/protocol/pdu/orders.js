...
OrderParser.prototype.parseDeltaRects = function(s, numRects, baseRect) {
    var rects = [];
    var prev = Object.assign({}, baseRect);
    for (var i = 0; i < numRects; i++) {
        var rect = {
            left: prev.left + this.readPackedValue(s),
            top: prev.top + this.readPackedValue(s),
            width: prev.width + this.readPackedValue(s),
            height: prev.height + this.readPackedValue(s)
        };
        rects.push(rect);
        prev = rect;
    }
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x80) {
        ...
        var readLength = s.offset - dataStart;
        if (readLength !== state.cbData) {
            log.warn('MultiOpaqueRect: cbData (' + state.cbData + ') does not match read length (' + readLength + '). Forcing sync.');
            s.offset = dataStart + state.cbData;
        }
    }
