...
OrderParser.prototype.parsePrimaryOrder = function(s, controlFlags) {
    ...
    if (!(controlFlags & ControlFlags.TS_ZERO_FIELD_COUNT)) {
        ...
        if (byte & 0x80) {
            ...
            s.offset++;
            fieldFlags |= (byte & 0x7F) << 7;
            if (byte & 0x80) {
                byte = s.buffer.readUInt8(s.offset);
                s.offset++;
                fieldFlags |= (byte & 0x7F) << 14;
            }
        }
    }
    ...
    switch(orderType) {
        ...
        case OrderType.TS_NEG_MULTIOPAQUERECT_INDEX:
            // MultiOpaqueRect shares state with OpaqueRect
            var sharedState = this.orderState[OrderType.TS_NEG_OPAQUERECT_INDEX];
            this.parseMultiOpaqueRect(s, controlFlags, fieldFlags, order.fields, sharedState);
            break;
        default:
...
OrderParser.prototype.readPackedValue = function(s) {
    ...
    if (byte & 0x80) {
        value |= (s.buffer.readUInt8(s.offset) << 6);
        s.offset++;
    }
    return isNegative ? -value : value;
};

OrderParser.prototype.parseDeltaRects = function(s, numRectangles, cbData, state) {
    var rects = [];
    var startOffset = s.offset;
    var left = state.nLeftRect;
    var top = state.nTopRect;
    var width = state.nWidthRect;
    var height = state.nHeightRect;
    ...
    if (readLength !== cbData) {
        log.warn("MultiOpaqueRect: read " + readLength + " bytes, but cbData is " + cbData);
...
OrderParser.prototype.parseMultiOpaqueRect = function(s, controlFlags, fieldFlags, fields, state) {
    ...
    if (fieldFlags & 0x80) {
        fields.rectangles = this.parseDeltaRects(s, state.numRectangles, state.cbData, state);
    }
