...
function Global(transport, fastPathTransport) {
	...
	this.orderParser = new (require('./orders').OrderParser)();
}
...
Client.prototype.recvFastPath = function (secFlag, s) {
	while (s.availableLength() > 0) {
		...
		case data.FastPathUpdateType.FASTPATH_UPDATETYPE_ORDERS:
			var orders = this.orderParser.parse(new type.Stream(pdu.obj.updateData.obj.orders.value), pdu.obj.updateData.obj.numberOrders.value);
...
Client.prototype.readUpdateDataPDU = function (updateDataPDU) {
	...
	case data.UpdateType.UPDATETYPE_ORDERS:
		var orders = this.orderParser.parse(new type.Stream(updateDataPDU.obj.updateData.obj.orders.value), updateDataPDU.obj.updateData.obj.numberOrders.value);
		this.emit('orders', orders);
		break;
...
Client.prototype.sendConfirmActivePDU = function () {
    ...
    // Set supported orders
    var orderSupport = orderCapability.orderSupport.value;
    orderSupport[caps.Order.TS_NEG_PATBLT_INDEX] = 1;
    orderSupport[caps.Order.TS_NEG_SCRBLT_INDEX] = 1;
    orderSupport[caps.Order.TS_NEG_MEMBLT_INDEX] = 1;
    orderSupport[caps.Order.TS_NEG_OPAQUERECT_INDEX] = 1;
