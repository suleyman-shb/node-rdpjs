...
function macData(macSaltKey, data, count) {
	...
	sha1.update(data);
	if (count !== undefined) {
		sha1.update(new type.UInt32Le(count).toStream().buffer);
	}
	var sha1Digest = sha1.digest();
...
Client.prototype.connect = function(gccClient, gccServer, userId, channels, selectedProtocol) {
	...
	if (selectedProtocol === 0) { // PROTOCOL_RDP
		var serverSecurity = gccServer.blocks.obj.find(function(e) {
			return e.obj.data.obj.__TYPE__ === gcc.MessageType.SC_SECURITY;
		}).obj.data.obj;
