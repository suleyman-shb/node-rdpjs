 ...
 * @returns {type.Component}
 */
function ordersUpdateDataPDU(opt) {
	var self = {
		__UPDATE_TYPE__ : UpdateType.UPDATETYPE_ORDERS,
		pad2Octets : new type.UInt16Le(),
		numberOrders : new type.UInt16Le(),
		pad2Octets2 : new type.UInt16Le(),
		orders : new type.BinaryString(null, { readLength : new type.CallableValue(function() {
			return opt.readLength.value - 6;
		}) })
	};
...
function updateDataPDU(updateData, opt) {
	var self = {
		...
		updateData : updateData || new type.Factory(function(s) {
			...
			case UpdateType.UPDATETYPE_ORDERS:
				self.updateData = ordersUpdateDataPDU(options).read(s);
				break;
 ...
 * @returns {type.Component}
 */
function fastPathOrdersUpdateDataPDU(opt) {
	var self = {
		__FASTPATH_UPDATE_TYPE__ : FastPathUpdateType.FASTPATH_UPDATETYPE_ORDERS,
		numberOrders : new type.UInt16Le(),
		orders : new type.BinaryString(null, { readLength : new type.CallableValue(function() {
			return opt.readLength.value - 2;
		}) })
...
function fastPathUpdatePDU (updateData, opt) {
	var self = {
		...
		updateData : updateData || new type.Factory( function (s) {
			...
			case FastPathUpdateType.FASTPATH_UPDATETYPE_ORDERS:
				self.updateData = fastPathOrdersUpdateDataPDU(options).read(s);
				break;
