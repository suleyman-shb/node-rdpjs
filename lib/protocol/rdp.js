...
var pdu = require('./pdu');
var credssp = require('./credssp');
var spnego = require('./spnego');
var ntlm = require('../security/ntlm');
...
function RdpClient(config) {
	...
	this.mcs.on('connect', function (gccClient, gccServer, userId, channels, selectedProtocol) {
		self.sec.connect(gccClient, gccServer, userId, channels, selectedProtocol);
	});
	...
	this.global.on('connect', function () {
		...
		self.emit('close');
	}).on('orders', function (orders) {
		self.emit('orders', orders);
...
RdpClient.prototype.connect = function (host, port) {
	...
	this.x224.on('connect', function(selectedProtocol) {
		if (selectedProtocol === 0x02) { // PROTOCOL_HYBRID
			self.handshakeNLA(function(err) {
				if (err) {
					log.error('NLA handshake failed: ' + err.message);
					self.emit('error', err);
					return;
				}
				self.mcs.connect(selectedProtocol);
			});
		} else {
			self.mcs.connect(selectedProtocol);
		}
...
RdpClient.prototype.handshakeNLA = function (callback) {
	var self = this;
	var ntlmNegotiate = ntlm.createNegotiateMessage();
	var spnegoToken = new spnego.NegTokenInit(ntlmNegotiate).encode();
	var tsRequest = new credssp.TSRequest(2, new credssp.NegoData(spnegoToken));

	var userName = this.sec.infos.obj.userName.value.toString('ucs2').replace(/\0/g, '');
	var password = this.sec.infos.obj.password.value.toString('ucs2').replace(/\0/g, '');
	var domain = this.sec.infos.obj.domain.value.toString('ucs2').replace(/\0/g, '');

	var socket = this.bufferLayer.socket;
	if (this.bufferLayer.securePair) {
		socket = this.bufferLayer.securePair.cleartext;
	}

	// Pause BufferLayer to receive raw data
	this.bufferLayer.paused = true;

	var onData = function(data) {
		var s = new type.Stream(data);
		var tsResponse;
		try {
			tsResponse = credssp.decodeTSRequest(s);
		} catch (e) {
			self.bufferLayer.removeListener('raw-data', onData);
			self.bufferLayer.paused = false;
			callback(new error.ProtocolError('NODE_RDP_PROTOCOL_NLA_DECODE_ERROR', 'Failed to decode NLA TSRequest: ' + e.message));
			return;
		}

		if (tsResponse.negoData) {
			var spnegoToken;
			try {
				spnegoToken = spnego.decodeSpnegoToken(new type.Stream(tsResponse.negoData.data));
			} catch (e) {
				self.bufferLayer.removeListener('raw-data', onData);
				self.bufferLayer.paused = false;
				callback(new error.ProtocolError('NODE_RDP_PROTOCOL_SPNEGO_DECODE_ERROR', 'Failed to decode SPNEGO token: ' + e.message));
				return;
			}

			var challenge;
			try {
				challenge = ntlm.decodeChallengeMessage(spnegoToken.responseToken);
			} catch (e) {
				self.bufferLayer.removeListener('raw-data', onData);
				self.bufferLayer.paused = false;
				callback(new error.ProtocolError('NODE_RDP_PROTOCOL_NTLM_CHALLENGE_ERROR', 'Failed to decode NTLM challenge: ' + e.message));
				return;
			}

			var authMsg = ntlm.createAuthenticateMessage(
				userName,
				password,
				domain,
				challenge
			...
			var spnegoAuth = new spnego.NegTokenResp(authMsg).encode();
			var tsAuth = new credssp.TSRequest(2, new credssp.NegoData(spnegoAuth));
			socket.write(tsAuth.encode());
		} else {
			self.bufferLayer.removeListener('raw-data', onData);
			// Resume BufferLayer
			self.bufferLayer.paused = false;
	...
	this.bufferLayer.on('raw-data', onData);
	socket.write(tsRequest.encode());
};
