...
var NTLMSSP_SIGNATURE = Buffer.from('NTLMSSP\0', 'binary');
...
function createAuthenticateMessage(userName, password, domain, challenge) {
    var domainBuf = Buffer.from(domain || '', 'ucs2');
    userName = userName || '';
    var userBuf = Buffer.from(userName, 'ucs2');
    var workstationBuf = Buffer.from('WORKSTATION', 'ucs2');

    var ntlmResponse = calculateNTLMv2Response(domain, userName, password, challenge.serverChallenge, challenge.targetInfo);
    var lmResponse = Buffer.alloc(24);
    lmResponse.fill(0);
...
function calculateNTLMv2Response(domain, user, password, serverChallenge, targetInfo) {
    var ntlmHash = md4(Buffer.from(password, 'ucs2'));
    var ntlmRev2Hash = crypto.createHmac('md5', ntlmHash).update(Buffer.from(user.toUpperCase() + domain.toUpperCase(), 'ucs2')).digest();

    var clientNonce = crypto.randomBytes(8);
    var timestamp = Buffer.alloc(8);
    ...
    var temp = Buffer.concat([
        Buffer.from([1, 1, 0, 0]), // respType, hiRespType, reserved1, reserved2
        Buffer.from([0, 0, 0, 0]), // reserved3
        timestamp,
        clientNonce,
        Buffer.from([0, 0, 0, 0]), // reserved4
        targetInfo,
        Buffer.from([0, 0, 0, 0]) // reserved5
    ]);
